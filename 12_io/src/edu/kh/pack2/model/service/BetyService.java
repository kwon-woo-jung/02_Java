...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		// 텍스트 파일을 작성 할 수 있는 메서드
		
		// FileOutputStream 참조 변수 선언 ( OutputStream 을 상속 받은 자식 클래스 )
		FileOutputStream fos = null;
		// 파일 출력: FileOutputStream 을 사용해 파일에 데이터를 바이트 단위로 출력 하는 메서드입니다.
		...
		try {
			...
			/*
			 StringBuilder를 사용해 여러 줄의 텍스트를 만들고, 이를 getBytes() 메서드를 사용해 바이트 배열로
			 변환 후 파일에 씁니다. 성능을 측정하기 위해 System.nanoTime() 으로 시작과 끝의 시간을 측정합니다
			*/
...
//			System.nanoTime()으로 시작과 끝의 시간을 측정합니다.
			
			fos.write(content.getBytes());
			// getBytes() 메서드를 사용해 바이트 배열로 변환 후 파일에 씁니다.
...
//		텍스트 파일을 작성할 수 있는 메서드 2
		
		/*BufferedOutputStream을 사용해 출력 성능을 개선한 방법입니다.
		작은 데이터를 모아서 한 번에 파일에 출력하므로 입출력 속도가 향상됩니다. 
		*/
		
		/*
		 StringBuilder를 사용해 텍스트를 만들고,
		BufferedOutputStream을 이용해 출력합니다.
		버퍼를 사용한 경우 더 빠른 성능을 보일 수 있습니다.
		*/
		...
		// 보조 스트림을 사용하여 데이터를 모아서 한 번에 입출력 합니다.
...
//			System.nanoTime()으로 시작과 끝의 시간을 측정합니다.
	...
	public void fileByteInput() {
		...
		// FileInputStream을 사용해 바이트 단위로 파일을 읽는 메서드입니다.		
		...
		try {
			...
			while(true) {
				...
				/*fis.read()로 1바이트씩 데이터를 읽고,
				이를 StringBuilder에 추가해 출력합니다.
				바이트 기반 스트림을 사용하면
				한글과 같은 2바이트 문자들이 깨질 수 있습니다.
				*/
	...
	public void fileByteInput2() {
		...
		/*파일 입력 (전체 읽기): FileInputStream의 readAllBytes() 메서드를 사용해
		파일의 모든 내용을 한 번에 바이트 배열로 읽고
		이를 문자열로 변환하는 방식입니다.
		이 방식은 파일이 작은 경우 더 간단하게
		파일을 처리할 수 있습니다.
		*/
	...
	// 파일 입력: FileInputStream을 사용해 바이트 단위로 파일을 읽는 메서드입니다.
		
	/*
	   버퍼를 이용한 파일 입력: BufferedInputStream을 사용하여 성능을 개선한 파일 입력 방식입니다.
	   버퍼를 사용해 데이터를 모아서 처리하므로 파일 읽기 속도가 빨라집니다
	*/
	...
	public void fileCopy() {
		...
		/*  파일 복사: 사용자가 입력한 파일 경로를 통해 파일을 읽고,
   			같은 경로에 복사본을 생성하는 메서드입니다.
   			복사본 파일은 원본 파일 이름에 "_copy"를 붙여 생성됩니다.ㄹ
		 */
		...
		try {
			...
			/*  파일 복사: 사용자가 입력한 파일 경로를 통해 파일을 읽고,
   			같은 경로에 복사본을 생성하는 메서드입니다.
   			복사본 파일은 원본 파일 이름에 "_copy"를 붙여 생성됩니다.
			 */
